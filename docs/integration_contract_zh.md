...
| `src.llm` | `ChatMessage`, `ToolCall`, `ChatResponse` | 仅作类型 / 渲染用，不要直接构造 Provider |
...
**GUI 不应自己拼装 `Agent`**。后端提供一个工厂函数（建议放在 `src/core/bootstrap.py`，由后端 lane 实现）：

```python
def build_agent(*, offline: bool = False) -> Agent: ...
```

`offline=True` 时挂 `EchoLLMProvider` + 离线工具子集，便于 GUI 在没有 API Key 时也能跑通。GUI 只调 `build_agent()`，不应感知 `DeepSeekProvider`、`PKU3bAssignmentsTool` 等具体子类的存在 —— 这些会在演示窗口内频繁增删。
...
| --- | --- | --- | --- |
| `llm_response` | `text: str` | 每次 LLM 回复后立即触发，包括携带 tool_calls 的中间回复 | 累计或暂存；最终展示由 `final` 决定 |
...
| `final` | `text: str` | 整个 turn 结束，`text` 是最终对用户可见的回答 | 在对话面板渲染最终消息 |
...
- 当前不支持 token 级流式（`LLMProvider.chat()` 一次性返回全文）。如果 v2 需要打字机效果，再加一个 `kind="llm_token"` 的事件，现有 GUI 代码不必改动。
...
- **token 级流式**：当前 `ChatResponse` 是一次性返回。如果对话面板要"打字机"效果，需要后端给 `LLMProvider` 加一个 `chat_stream()` 方法 + 新事件 `llm_token`。
