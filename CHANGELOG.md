...
### Added
- Treehole context guards, so the agent's `treehole` tool can't flood the conversation. `TreeholeTool` now splits a query into a few whitespace keywords (≤3, deduped/clipped), searches each, merges + ranks candidates (relevance / replies / likes / freshness, with a `sort` mode and a lightweight `category` filter), and returns clipped per-hole summaries; `fetch` returns a bounded recent-comment preview (`comment_mode`, default ≤8) instead of the full thread, and `_fit_treehole_payload` caps the JSON envelope. `Agent.turn()` gains three matching guards: a context-length API error is caught and turned into a friendly "新开对话" final instead of crashing; a repeated identical tool call (normalized signature) stops the turn to break loops; and `treehole` `search`/`fetch` are each capped at once per turn. Tool results are now serialized as compact, truncated JSON (≤3–4k chars) rather than `str(data)`. Backend-lane integration call: the originally-bundled proactive history char-budget trimming was dropped — it stripped `reasoning_content` from every replayed message, diverging from the DeepSeek thinking-mode round-trip invariant for no real benefit; only the reactive context-length guard is kept, so prior `reasoning_content` is preserved intact (verified by `scripts/smoke_deepseek.py`). Tests: `tests/test_agent_guards.py`, `tests/test_treehole_updates_tool.py`. (`codex/treehole-context-guards`, by-lastime; backend-lane integration by captain)
