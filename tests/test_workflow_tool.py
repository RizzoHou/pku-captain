...
def test_demo_workflow_opts_out_of_agent_tools() -> None:
    ...
    assert _FailingWorkflow(ToolRegistry()).agent_callable is True
    assert HelloWorkflow(ToolRegistry()).agent_callable is False
    assert Workflow.agent_callable is True


def test_build_agent_filters_non_callable_workflow_stub() -> None:
    # The gate in action: `_register_workflow_tools` exposes only agent-callable
    # workflows to the LLM tool schema; the `hello` demo stub (agent_callable=
    # False) is filtered out. Offline so no API key is needed.
    agent = build_agent(offline=True)

    schema_names = {t["function"]["name"] for t in agent.tools.to_openai_schema()}

    # The demo stub is NOT exposed to the agent as a tool...
    assert "hello" not in agent.tools
    assert "hello" not in schema_names

    # ...yet still lives in the WorkflowRegistry, so the GUI button path and the
    # offline reference stub are untouched.
    assert {w.name for w in agent.workflows.all()} == {"hello"}
